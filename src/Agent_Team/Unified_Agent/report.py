...
def normalize_source_domains(output: dict[str, Any], context: dict[str, Any]):
    """Derive display metadata from citations without changing prose or sources."""
    from jsonschema import Draft202012Validator
    Draft202012Validator(response_format(context)["schema"]).validate(output)
    catalog = evidence_catalog(context)
    result = copy.deepcopy(output)
    changes = []
    for group in ("findings", "risks"):
        for index, finding in enumerate(result[group]):
            cited = {catalog[key]["domain"] for key in finding["evidence_ids"]}
            domains = [domain for domain in ("financial", "news", "market") if domain in cited]
            if set(finding["domains"]) != cited:
                changes.append({"location": f"{group}[{index}]", "original_domains": finding["domains"], "cited_domains": domains})
