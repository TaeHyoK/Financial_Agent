...
The end-to-end loop runs this as a subprocess with ``PYTHONPATH=src``.
"""
...
def main() -> None:
    ...
    parser.add_argument("--market-json", required=True, help="Primary YFinance market_full_dataset JSON.")
    parser.add_argument("--dart-json", required=True, help="Supporting DART lightweight JSON.")
    parser.add_argument("--news-json", required=True, help="Supporting News period summaries JSON.")
    parser.add_argument("--valuation-json", default=None, help="Point-in-time YFinance valuation snapshot JSON.")
    parser.add_argument("--report-md", default=str(DEFAULT_REPORT_MD), help="Markdown report output path.")
    parser.add_argument("--report-json", default=str(DEFAULT_REPORT_JSON), help="Structured JSON report output path.")
    ...
    parser.add_argument("--model", default=None, help="OpenAI model for LLM report generation. Defaults to OPENAI_MODEL or gpt-5.4.")
    parser.add_argument("--env-file", default=str(DEFAULT_ENV_PATH), help="Env file to load before calling OpenAI.")
    parser.add_argument(
        "--primary-data-only",
        action="store_true",
